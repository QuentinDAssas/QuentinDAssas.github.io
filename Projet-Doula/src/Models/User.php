...
class User extends Model
{
    ...
    */
    ...
    {
        $query = $this->pdo->prepare("SELECT Id, LastName, FirstName, Email, Created_at, Last_login, Admin FROM $this->table");
    ...
    {
       $query = $this->pdo->prepare("SELECT Admin FROM $this->table WHERE Id = :id");
    ...
    {
       $query = $this->pdo->prepare("SELECT FirstName, LastName, Email, Address, City, Phone FROM $this->table WHERE Id = :id");
    ...
    {
        $query = $this->pdo->prepare("SELECT Id FROM $this->table WHERE Email LIKE :email");
        $query->execute([
            ':email' => $email,
        ]);
    ...
    {
        //vérifier si il s'agit d'un compte administrateur
        if ($u['admin']==true)
        {
            $u['admin']=1;
        }else{
    ...
    public function verifEmailPwd(string $email,string $pwd):bool
    {
        ...
        $query = $this->pdo->prepare("SELECT Id,FirstName,Password,Admin FROM $this->table WHERE Email LIKE :email LIMIT 0,1");
        $query->execute([
            ':email' => $email,
        ]);
